...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.save()
            userprofile = UserProfile(user = user, profile_picture='userprofile/default.jpg')
            userprofile.save()
            ...
            send_email['From'] = 'variedadeseliza@outlook.com'
            ...
            with smtplib.SMTP("smtp.outlook.com", port=587) as smtp:
                ...
                smtp.login(send_email["From"], "Bodegon_")
        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    #get the cart item by cart id
                    items_cart=[]
                    for item in cart_item:
                        items_cart.append(item)

                    #get the cart items from the user to access his products
                    cart_item_user = CartItem.objects.filter(user=user)

                    items_user=[]
                    items_id_user=[]
                    for item in cart_item_user:
                        items_user.append(item) 
                        items_id_user.append(item.product.id)

                    for pr in items_cart:
                        if pr.product.id in items_id_user:
                            index = items_id_user.index(pr.product.id)
                            item_id=items_user[index].id
                            item = CartItem.objects.get(id=item_id)
                            item.quantity += 1
                            item.user = user
                            item.save()
                        else:
                            cart_item = CartItem.objects.filter(cart=cart, id=pr.id)
                            for item in cart_item:
                                item.user = user
                                item.save()
            except:
...
def order_detail(request, order_id):
    ...
    template = get_template('includes/orderpdf.html')
